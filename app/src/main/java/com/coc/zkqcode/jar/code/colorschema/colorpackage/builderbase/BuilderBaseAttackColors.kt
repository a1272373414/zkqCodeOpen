...
object BuilderBaseAttackColors : IBuilderBaseAttackColors {
    ...
    // 部队技能粉条：与英雄充能条同一套粉色，并且同样是【上中下垂直叠色】（用户指正），
    // 采样点沿垂直方向取（x 不变、y 递增）。
    // 注意 **颜色串是 BGR**：实机 RGB 采样 570:EE5AFF / 574:C534FD / 578:C73CFE / 582:C946FF / 590:D77AFF，
    // 转为 BGR 即 FF5AEE / FD34C5 / FE3CC7 / FF46C9 / FF7AD7。
    // （本题原定义用的 FF44C9 恰好就是其中一层的 BGR，之前误按 RGB 解释才把颜色改坏。）
    // 未充能（进度0）整条是灰 252525，不会命中。条横跨整个兵卡行（y 570~594）。
    override val TroopSkills = ColorSchema.parse(
        189, 566, 1241, 600, "FF5AEE", "0|4|FD34C5,0|8|FE3CC7,0|12|FF46C9,0|20|FF7AD7", 0, 0.9, "开部队技能"
    )
    ...
    // 战争机器技能就绪 = 英雄卡槽上方的充能条"长满到最右端"。
    // 由实机截图（I:\coc\游戏截图\夜世界-英雄充能\进度0~3、进度31）逐像素采样得出：
    //   条固定在 y≈561~567、从 x=93 向右生长，共 3 格（每格约 22px）——
    //   满 1 格 ≈ x 93~113、满 2 格 ≈ x 93~134、满 3 格 ≈ x 93~159。
    // 两个要点：
    //   ① 这个条不是左右渐变，而是【上中下三层叠色】，采样点必须沿垂直方向取；
    //   ② **颜色串是 BGR 顺序**（见 ColorSchema.parseBgrToRgb），下面是实机 RGB 采样转成的 BGR。
    //   未充能(进度0)：RGB 31AFFF/4ABFF0/A1F1FF（蓝）→ 不会命中
    //   已充能(进度1起)：RGB C125FB/C73AFE/DF98FF（粉）→ BGR FB25C1/FE3AC7/FF98DF
    // 其中 FE3AC7 与本项目既有的"英雄技能就绪粉光"BattleCopterSkillsAlt 完全一致，可作旁证。
    // 用户实机确认：**满 1 格即可释放**，所以只判第 1 格 (x 93~116) 的三层是否为粉。
    override val HeroChargeReady = ColorSchema.parse(
        93, 559, 116, 570, "FB25C1",
        "0|3|FE3AC7,0|6|FF98DF",
        0, 0.9, "英雄充能就绪"
    )
